...
- 重构 `tools/unit_generator.clj`（#29）：添加 `:pre [(sequential? units)]` 输入校验；模板渲染前检查资源存在性，缺失时抛出携带上下文信息的 `ex-info`；移除未使用的 `gsein-war3.lni.available-id` require；清理 comment 块中的项目特定硬编码数据；提取模板路径为命名常量；新增单元测试覆盖空列表、非法参数与正常渲染；构建与测试通过
- 重构 `tools/title_generator.clj`（#27）：移除激进的 `delete-files-in-dir`（改为直接覆盖生成，避免误删用户数据）；修复 `create-img` 与 `get-scaled-image` 中 Graphics2D 资源泄漏（try/finally 确保 dispose）；提取硬编码默认值为命名常量（字体、尺寸、模板路径等）；移除 `println`；修复 `get-file-pinyin-name-without-extension` 无扩展名时 `StringIndexOutOfBoundsException`；重命名 `generate-title-with-image!` 参数 `img` → `img-path`；清理过长 comment 块；构建与测试通过
