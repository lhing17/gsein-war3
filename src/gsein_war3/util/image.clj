  ...
  (let [result (copy origin)]
    (if opts
      (case (:filter-type (first opts))
        :none result
        :default (do-add-border! result (try (jio/input-stream (jio/resource "blp/active_border2.png"))
                                             (catch IOException _ (throw (IllegalStateException. "未找到默认滤镜，请使用自定义滤镜")))))
        :self-defined (do-add-border! result (try (jio/input-stream (:filter-url (first opts)))
                                                  (catch IOException _ (throw (IllegalStateException. "滤镜图片无效")))))
        )
      result
      )
    )
...
(defn add-passive-border [origin]
  (let [result (copy origin) border (ImageIO/read (jio/resource "blp/passive_border.png"))]
    (loop [start 0 end 63]
      (if (< start end)
...
(defn output-as-blp [^BufferedImage image dir name prefix]
  (let [path (Paths/get dir (into-array String [])) blp-path (.resolve path (str prefix name ".blp"))]
